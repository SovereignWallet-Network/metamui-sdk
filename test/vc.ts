...
describe('VC works correctly', () => {
  ...
  it('VC is created in correct format', async () => {
    ...
    actualHex = await vc.generateVC(tokenVC, owner, issuers, VCType.TokenVC, sigKeypair);
  ...
  it('Token VC creation fails token name is not given', async () => {
    ...
    try {
      await vc.generateVC(tokenVC, owner, issuers, VCType.TokenVC, sigKeypairValidator);
    } catch (e) {
